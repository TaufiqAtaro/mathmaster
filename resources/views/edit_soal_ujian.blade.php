    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-black text-2xl text-gray-800 leading-tight flex items-center gap-2">
                ...
                Edit Soal Ujian Akhir
            </h2>
            <a href="/kelola-ujian/{{ $soal->modul_id }}" class="text-gray-500 hover:text-pink-600 font-bold transition flex items-center gap-1 text-sm">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <form action="/ujian/{{ $soal->id }}" method="POST" class="bg-white p-8 rounded-2xl shadow-sm border-t-4 border-pink-500 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="pertanyaan" rows="4" class="w-full border-gray-300 rounded-xl p-3 focus:ring-pink-500 shadow-sm" required>{{ old('pertanyaan', $soal->pertanyaan) }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="opsi_a" value="{{ old('opsi_a', $soal->opsi_a) }}" class="w-full border-gray-300 rounded-xl p-3 focus:ring-pink-500 shadow-sm" required>
                    </div>
                    <div>
                        <label class="block font-bold text-gray-700 mb-2">Opsi B <span class="text-red-500">*</span></label>
                        <input type="text" name="opsi_b" value="{{ old('opsi_b', $soal->opsi_b) }}" class="w-full border-gray-300 rounded-xl p-3 focus:ring-pink-500 shadow-sm" required>
                    </div>
                    <div>
                        <label class="block font-bold text-gray-700 mb-2">Opsi C <span class="text-red-500">*</span></label>
                        <input type="text" name="opsi_c" value="{{ old('opsi_c', $soal->opsi_c) }}" class="w-full border-gray-300 rounded-xl p-3 focus:ring-pink-500 shadow-sm" required>
                    </div>
                    <div>
                        <label class="block font-bold text-gray-700 mb-2">Opsi D <span class="text-red-500">*</span></label>
                        <input type="text" name="opsi_d" value="{{ old('opsi_d', $soal->opsi_d) }}" class="w-full border-gray-300 rounded-xl p-3 focus:ring-pink-500 shadow-sm" required>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="pembahasan" rows="3" class="w-full border-gray-300 rounded-xl p-3 focus:ring-pink-500 shadow-sm">{{ old('pembahasan', $soal->pembahasan) }}</textarea>
                </div>
